        ...
        <div class="culmn">
            ...
            <div class="container">
                <div class="row">
                  ...
                  <?php
                      if($range=="author"){
                        $result_list = mysqli_query($conn, "SELECT * FROM note_recv WHERE recv_id='$userid' ORDER BY send_id, idx DESC");
                      } else if($range=="content") {
                        $result_list = mysqli_query($conn, "SELECT * FROM note_recv WHERE recv_id='$userid' ORDER BY content, idx DESC");
                      } else {
                        $result_list = mysqli_query($conn, "SELECT * FROM note_recv WHERE recv_id='$userid' ORDER BY idx, send_id DESC");
                      }
                      $scale = 10;

                      $total_record = mysqli_num_rows($result_list); //전체 글 수
                      /*
                      echo "SELECT * FROM note_recv WHERE send_id='$userid' ORDER BY send_id, idx DESC";
                      echo $total_record;
                      exit();
                      */
                  ...
                  <div class="col-sm-12">
                    <div class="col-sm-3">
                        <form method="post" action="note_recv.php?kind=<?php echo $kind; ?>">
                            <select onchange="location = this.value;" class="form-control" style="display: inline-block;">
                            <option value="note_recv.php?range=num">날짜</option>
                            <option value="note_recv.php?range=author">작성자</option>
                            <option value="note_recv.php?range=content">내용</option>
                            </select>
                        </form>
                    </div>
                ...
                <?php
                  for($i = $start; $i < $start+$scale && $i < $total_record; $i++){
                    ...
                    $note_send_id = $recv_row['send_id'];
                    $note_chk = $recv_row['recv_chk'];
                ...
                <!-- <script type="text/javascript">
                  ...
                  function read(){
                    // num = $("#idx").val();
                    // alert(<?php echo $note_idx;?>);
                    alert("메세지 확인이 완료 되었습니다!");
                    <?php
 ?>
                  }
                </script> -->
